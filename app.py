...
Streamlit + Plotly를 사용한 웹 애플리케이션
"""
...
with col1:
    original_count = len(df) if selected_position == 'All' else len(df[df['Position_Category'] == selected_position])
...
with tab1:
    if len(df_filtered) == 0:
        ...
        with col_info:
            if selected_position != 'All':
                st.info(f"📌 **{selected_position}** 포지션 {len(df_filtered):,}명 | 💡 **왼쪽 차트에서 선수를 클릭**하면 오른쪽에 능력치가 표시됩니다!")
            else:
        ...
        with col_ranking:
            ...
            if active_stats and selected_position != 'All':
                ...
                if max_score > min_score:
                    df_score['Display_Score'] = ((df_score['Filter_Score'] - min_score) / (max_score - min_score) * 100)
                else:
                ...
                applied_stats = [position_key_stats[selected_position][s]['label'] for s in active_stats if s in position_key_stats.get(selected_position, {})]
...
with tab2:
    ...
    if len(df_filtered) == 0:
        ...
        if selected_position == 'Goalkeeper':
            compare_attrs = ['Age', 'Reflexes', 'Handling', 'OneOnOnes', 'CommandOfArea', 'Kicking', 'Agility', 'Talent_Score_Normalized']
        elif selected_position == 'Defender':
            compare_attrs = ['Age', 'Marking', 'Tackling', 'Heading', 'Positioning', 'Pace', 'Strength', 'Anticipation', 'Talent_Score_Normalized']
        elif selected_position == 'Midfielder':
            compare_attrs = ['Age', 'Passing', 'Vision', 'Technique', 'Stamina', 'Workrate', 'Dribbling', 'FirstTouch', 'Talent_Score_Normalized']
        elif selected_position == 'Forward':
            compare_attrs = ['Age', 'Finishing', 'Dribbling', 'Pace', 'Acceleration', 'Composure', 'OffTheBall', 'Technique', 'Talent_Score_Normalized']
        else:
            compare_attrs = ['Age', 'Overall_Rating', 'Technical_Rating', 'Mental_Rating', 'Physical_Rating', 'Pace', 'Passing', 'Finishing', 'Talent_Score_Normalized']
